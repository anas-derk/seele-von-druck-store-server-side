...
const { validateJWT, validateNumbersIsGreaterThanZero, validateNumbersIsNotFloat, validateSortMethod, validateSortType, validateIsExistErrorInFiles, validateIsPriceGreaterThanDiscount } = require("../middlewares/global.middlewares");
...
productsRouter.post("/add-new-product",
    ...
    (req, res, next) => {
        const { name, price, description, category, discount, tax, quantity, isStockDisplayed } = Object.assign({}, req.body);
        validateIsExistValueForFieldsAndDataTypes([
            ...
            { fieldName: "Is Stock Displayed", fieldValue: isStockDisplayed, dataType: "boolean", isRequiredValue: false },
        ], res, next);
    },
    (req, res, next) => {
...
productsRouter.put("/:productId",
    validateJWT,
    (req, res, next) => {
        const { name, price, description, category, discount, tax } = req.body;
        validateIsExistValueForFieldsAndDataTypes([
            { fieldName: "Product Id", fieldValue: req.params.productId, dataType: "ObjectId", isRequiredValue: true },
            { fieldName: "Name", fieldValue: name, dataType: "string", isRequiredValue: true },
            { fieldName: "Price", fieldValue: Number(price), dataType: "number", isRequiredValue: true },
